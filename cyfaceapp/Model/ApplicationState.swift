...
class ApplicationState: ObservableObject {
    ...
    // TODO: var dcs: DataCapturing.Measurement
    ...
    init(settings: Settings) {
        ...
        do {
            ...
            // TODO: self.dcs = DataCapturingService(sensorManager: CMMotionManager(), dataManager: coreDataStack)
            /* TODO: try coreDataStack.setup(bundle: bundle) { [weak self] error in
                self?.isInitialized = true
            }
            self.dcs.handler.append(self.handle)
            self.dcs.setup() */

            // TODO: self.settings.add(serverUrlChangedListener: self)
            // TODO: self.settings.add(uploadToggleChangedListener: self)

            // TODO: let persistenceLayer = PersistenceLayer(onManager: coreDataStack)
            /* TODO: for measurement in try persistenceLayer.loadSynchronizableMeasurements() {
     ...
     */
    /* TODO: func startSynchronization(authenticator: CredentialsAuthenticator?) {
...
extension ApplicationState: UploadToggleChangedListener {
    func to(upload: Bool) {
        if upload && isLoggedIn {
            ...
            do {
                // TODO: try synchronizer?.activate()
            } catch {
                hasError = true
                errorMessage = error.localizedDescription
            }
...
/* TODO: extension ApplicationState: CyfaceEventHandler {
